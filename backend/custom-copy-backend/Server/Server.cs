...
using custom_copy_backend.Crypto;
...
namespace custom_copy_backend.Server
{      
    public class WebSocketServer
    {
        ...
        public async void Start(string listenerPrefix)
        {
            ...
            catch (Exception e)
            {
                ...
                Logs.Instance.PushError($"Custom Copy Server is already running or {listenerPrefix} is in use.");

                // Cannot continue!
                Environment.Exit(0);
            }
            Logs.Instance.Push("Start websocket server - " + listenerPrefix + "\r\n\tWaiting for custom-copy-frontend-client-ui connected...");
            ...
            while (true)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        var res = Encoding.UTF8.GetBytes("Custom Copy WebSocket Server<br>400 Bad Request");
